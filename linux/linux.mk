...
LINUX_TARBALL = $(call github,OpenNuvoton,MA35D1_linux-6.18.y,$(call qstrip,$(BR2_TARGET_KERNEL_MA35_VERSION)))/MA35D1_linux-6.18.y-$(call qstrip,$(BR2_TARGET_KERNEL_MA35_VERSION)).tar.gz
LINUX_SITE = $(patsubst %/,%,$(dir $(LINUX_TARBALL)))
LINUX_SOURCE = $(notdir $(LINUX_TARBALL))
...
ifeq ($(BR2_LINUX_KERNEL_MA35_5_10_RT_VERSION),y)
define LINUX_MA35_APPLY_RT_PATCHES
	$(APPLY_PATCHES) $(LINUX_DIR) $(TOPDIR)/linux/5.10.140-rt
endef
LINUX_POST_PATCH_HOOKS += LINUX_MA35_APPLY_RT_PATCHES
else ifeq ($(BR2_LINUX_KERNEL_MA35_6_6_RT_VERSION),y)
define LINUX_MA35_APPLY_RT_PATCHES
	$(APPLY_PATCHES) $(LINUX_DIR) $(TOPDIR)/linux/6.6.93-rt
endef
LINUX_POST_PATCH_HOOKS += LINUX_MA35_APPLY_RT_PATCHES
endif
...
define LINUX_KERNEL_FEATURE_PATCHES
	$(if $(BR2_LINUX_KERNEL_MA35_6_6_VERSION), \
		$(APPLY_PATCHES) $(LINUX_DIR) $(TOPDIR)/linux/6.6.93 \*.patch)
	$(if $(BR2_LINUX_KERNEL_MA35_6_12_VERSION), \
		$(APPLY_PATCHES) $(LINUX_DIR) $(TOPDIR)/linux/6.12.y \*.patch)
	$(if $(BR2_LINUX_KERNEL_MA35_6_18_VERSION), \
		$(APPLY_PATCHES) $(LINUX_DIR) $(TOPDIR)/linux/6.18.y \*.patch)
endef

LINUX_POST_PATCH_HOOKS += LINUX_KERNEL_FEATURE_PATCHES
...
define LINUX_BUILD_CMDS
	$(if $(BR2_NUVOTON_MA35D1),$(if $(BR2_TARGET_KERNEL_DRM_MA35_VERSION),$(SED) 's/ma35d1\.dtsi/ma35d1-drm\.dtsi/' $(LINUX_ARCH_PATH)/boot/dts/$(addsuffix .dts,$(LINUX_DTS_NAME)),$(SED) 's/ma35d1-drm\.dtsi/ma35d1\.dtsi/' $(LINUX_ARCH_PATH)/boot/dts/$(addsuffix .dts,$(LINUX_DTS_NAME))))
